...
budget 65536, 2 s) for the accuracies where the step is smallest.
    ...
    uv run python -m part1.bench.probe_floor_occupancy
"""
...
SIM_S = 2.0
BUDGET = 65536
OUT = "part1/bench/results/tables/floor_occupancy.md"
...
def main() -> int:
    ...
    for scene in SCENES:
        for arm in ARMS:
            for tol in TOLS:
                r = subprocess.run(
                    [sys.executable, "-m", "part1.bench.probe_floor_occupancy", "--single", scene, arm, str(tol)],
                    capture_output=True,
                    text=True,
                    timeout=3600,
                )
