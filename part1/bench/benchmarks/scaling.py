...
"""N-scaling benchmark: wall time vs world count for adaptive/fixed/single_iter.

Standalone:
    uv run python -m part1.bench.benchmarks.scaling --ns 1 4 16 64 256
...
from part1.bench.infra import MeasureResult, measure, power_law_exponent
from part1.bench.plotting import STYLES, SeriesData, log_log_plot, save_fig
from part1.scenes.contact_objects import DT_OUTER, build_model_randomized, make_fixed_solver, make_solver

MODES = ["adaptive", "fixed", "single_iter"]
...
def _measure_mode(mode: str, n: int, steps: int, warmup: int, trials: int = 1) -> MeasureResult:
    ...
    def _single_trial() -> MeasureResult:
        if mode == "adaptive":
            ...
            from part1.bench.benchmarks._single_iter import measure_single_iter  # noqa: PLC0415
...
def _collect_error_trace(steps: int) -> dict:
    """Run N=1 adaptive mode and record accepted error + dt at each outer step."""
...
def run(ns: list[int], steps: int, warmup: int, trials: int = 1) -> dict:
    ...
    # wall time for adaptive (per_iter is biased by K correlation with N).
    data["exponents"] = {}
    for mode in MODES:
        if mode == "adaptive":
            data["exponents"][mode] = power_law_exponent(ns, data["modes"][mode]["medians"])
        else:
            data["exponents"][mode] = power_law_exponent(ns, data["modes"][mode]["per_iter_medians"])

    # Error trace (N=1, adaptive mode).
...
def plot(data: dict, out_dir: Path) -> None:
    ...
    # Plot 1: Wall time per outer step vs N (adaptive + fixed)
    fig, ax = plt.subplots(figsize=(10, 6))
    wall_modes = ["adaptive", "fixed"]
    series = {}
    ...
    # Divides adaptive wall time by K_mean so both modes show per-step cost.
    fig, ax = plt.subplots(figsize=(10, 6))
    norm_series = {}
    for mode in wall_modes:
        if mode not in modes_data:
    ...
    # Plot 2: Per-iteration cost vs N (adaptive + single_iter + fixed)
    fig, ax = plt.subplots(figsize=(10, 6))
    iter_modes = ["adaptive", "single_iter", "fixed"]
    per_iter_series = {}
    ...
    # Plot 3: Iteration count K vs N (adaptive only)
    if "adaptive" in modes_data:
        fig, ax = plt.subplots(figsize=(10, 6))
        style = STYLES["adaptive"]
        md = modes_data["adaptive"]
        ax.plot(
            ns,
            md["k_means"],
            color=style.color,
            ...
            lw=2,
            ms=5,
            label=f"{style.label}  $K_{{mean}}$",
        )
        if "k_p25s" in md and "k_p75s" in md:
            ax.fill_between(ns, md["k_p25s"], md["k_p75s"], color=style.color, alpha=0.10)
        ax.plot(
            ns,
            md["k_maxes"],
            color=style.color,
    ...
    # Plot 4: Cost per world vs N (adaptive + fixed)
    fig, ax = plt.subplots(figsize=(10, 6))
    amort_series = {}
    for mode in wall_modes:
        if mode not in modes_data:
    ...
    # Plot 5: Error vs simulation time (N=1, adaptive)
    ...
    print(f"{'=' * 78}")
    hdr = f"{'mode':>12}  {'exponent':>10}  {'N=1':>10}  {'N=' + str(ns[-1]):>10}  {'ratio':>6}  {'K_mean':>6}"
    print(hdr)
    print("-" * len(hdr))
    for mode in MODES:
        md = modes_data[mode]
        ...
        if mode == "adaptive":
            ...
            label = "per iter"
        print(f"{mode:>12}  N^{exp:<7.3f}   {t1:9.2f}  {tN:10.2f}  {tN / t1:5.1f}x  {k:6.1f}  ({label})")
...
def main():
    ...
    parser.add_argument(
        "--trials", type=int, default=1, help="Repeat each (N, mode) measurement and keep the best median."
    )
    parser.add_argument("--out-dir", type=str, default="part1/bench/results")
