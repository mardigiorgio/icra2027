...
Standalone:
    uv run python -m part1.bench.benchmarks.part1_scaling   # 2^6 .. 2^13 worlds
"""
...
from part1.bench.four_arms import ARMS, ExhaustionTracker, IterationTracker, build_model, make_arm, scene_dt_outer
...
def main() -> int:
    ...
    args = p.parse_args()
    out = args.out or f"part1/bench/results/part1_scaling_{args.scene}.csv"
    ...
    for arm_name in args.arms:
        for n in args.ns:
            ...
            for _ in range(args.trials):
                r = subprocess.run(
                    [
                        sys.executable, "-m", "part1.bench.benchmarks.part1_scaling",
                ...
                if "over the scannable budget" in r.stderr:
                    print(f"CONFIG FAILED {arm_name} n={n}: CONTACT OVERFLOW -- contacts dropped, raise the budgets in four_arms.py", flush=True)
                    break
