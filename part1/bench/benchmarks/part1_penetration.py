...
Standalone:
    uv run python -m part1.bench.benchmarks.part1_penetration --n 64 --steps 200
...
from part1.bench.four_arms import ARMS, ExhaustionTracker, build_model, make_arm, scene_dt_outer
from part1.scenes.cenic_scenes import BIN_HALF, BIN_WALL_H, SCENES
...
def _run_pass(scene: str, arm_name: str, knob, n: int, sim_s: float, warmup_s: float, seed: int, which: str, margin: float) -> dict:
    ...
    import part1.scenes.cenic_scenes as CS
...
def main() -> int:
    ...
    out = args.out or f"part1/bench/results/part1_penetration_{args.scene}{suffix}.csv"

    if args.single is not None:
        arm_name, knob_s, which = args.single
        knob = int(knob_s) if arm_name in ("mujoco", "icf") else float(knob_s)
        margin = args.margin if args.margin is not None else __import__("part1.scenes.cenic_scenes", fromlist=["CONTACT_MARGIN"]).CONTACT_MARGIN
    ...
    def run_pass(arm_name, knob, which):
        r = subprocess.run(
            [
                sys.executable, "-m", "part1.bench.benchmarks.part1_penetration",
