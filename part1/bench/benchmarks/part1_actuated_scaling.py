...
(part1/scenes/actuated_press.py) at 64..4096 heterogeneous worlds.
...
world per boundary (IterationTracker), exhausted and unstable fractions.
    ...
    uv run python -m part1.bench.benchmarks.part1_actuated_scaling
"""
...
from part1.bench.four_arms import ExhaustionTracker, IterationTracker, _make_icf, _make_icf_adaptive, _make_mujoco, _make_mujoco_adaptive
from part1.scenes.actuated_press import TIP_MASS, ActuatedSpec, build, program
...
def main() -> int:
    ...
    p.add_argument("--out", type=str, default="part1/bench/results/part1_actuated_scaling.csv")
    ...
    for n in args.ns:
        for backend in ("icf", "mujoco"):
            for kind, knob in (("adaptive", 1e-3), ("fixed", 1e-3)):
                r = subprocess.run([sys.executable, "-m", "part1.bench.benchmarks.part1_actuated_scaling", "--single", backend, kind, str(knob), str(n)], capture_output=True, text=True, timeout=7200)
                got = None
