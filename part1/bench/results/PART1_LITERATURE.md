...
**As built** (`part1_actuated.py`, the standing CSV) the sweep narrowed:
k = 10⁵ N/m only, 300 mm/s only, a side push at mid-height (no
press-from-above), ε ∈ {10⁻¹…10⁻⁴} with no ε = 10⁻⁶ reference arm;
chatter is box vertical-velocity RMS and tip–box relative-velocity RMS in
the cruise (no force high-pass), and there is no energy-injection metric.
The block above is the design memo, kept for provenance.
...
| Measured error vs cost (not requested ε) | Hairer–Wanner; Erez 2015 short-window consistency (Themes A, C) | `part1_consistency.py`, `figures/consistency.pdf` | with the reference-vs-itself floor row: soft clutter first-order for both, ICF error control at parity with fixed ICF at equal cost (1.0–1.1×), MuJoCo error control 2–3× below its own fixed line at tight ε; hard clutter's tight end sits within 2–4× of the noise floor and ranks nothing |
| Actuated stiff contact with the controller gain as the axis | CENIC Fig. 12; Drake #14694; pushing datasets (Theme E) | `actuated_press.py`, `part1_actuated.py`, `figures/actuated.pdf`, `figures/actuated_chatter.pdf` | ICF stable in all 40 cells and slides the box flat at every gain; MuJoCo unstable in 6 (explicit joint gain), its box rocks and hops in every stable cell (2–58 mm; 192 mm in the surviving ε = 10⁻¹ K_p = 10⁵ cell), error control does not remove it; the held 100 Hz target excites tip–box chatter at K_p ≥ 1e5 that both solvers resolve as the step shrinks and 10 ms hides |
| Run-to-run determinism as a certificate | SimBenchmark; Erez 2015 (Theme A) | `probe_determinism.py`, `tables/determinism_probe.md` | ball bit-exact on every arm; neither contact solver reproducible on clutter (ICF mm within 0.3 s soft, both cm within 0.5 s hard) — the consistency bench carries the reference-vs-itself floor row |
