...
through the solver constructor. The pyramidal rows come from the bench's CSV.
    ...
    uv run python -m part1.bench.probe_mujoco_cone
"""
...
BENCH_CSV = "part1/bench/results/part1_penetration_hard-clutter.csv"
OUT = "part1/bench/results/tables/mujoco_cone_probe.md"
...
def _single(arm_name: str, knob: float) -> dict:
    ...
    import part1.bench.benchmarks.part1_penetration as P
...
def main() -> int:
    ...
    for arm, knob in PASSES:
        ...
        res = subprocess.run(
            [sys.executable, "-m", "part1.bench.probe_mujoco_cone", "--single", arm, str(knob)],
            capture_output=True,
            text=True,
            timeout=3600,
        )
