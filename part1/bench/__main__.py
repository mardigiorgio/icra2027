...
"""Adaptive solver benchmark platform entry point.

Usage:
    uv run -m part1.bench                          # run all benchmarks
    uv run -m part1.bench --only scaling            # run one benchmark
    uv run -m part1.bench --skip accuracy           # skip one
    uv run -m part1.bench --list                    # list available benchmarks
    uv run -m part1.bench --ns 1 4 16 64 256        # override N values
    uv run -m part1.bench --steps 50 --warmup 20    # override timing params
"""

import argparse

from part1.bench.runner import list_benchmarks, run


def main():
    parser = argparse.ArgumentParser(
        description="Adaptive solver benchmark platform",
    ...
    parser.add_argument("--seed", type=int, default=None, help="RNG seed for randomized ICs in initial_conditions")
