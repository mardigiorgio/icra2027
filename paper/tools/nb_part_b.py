...
def partB(nb):
    ...
    nb.experiment("B3. Realized contact stiffness (figures/stiffness_sweep.pdf)",
        ...
        says=["ICF: ratio 1.00 at every k up to 10⁷ at δt = 10 ms, at 1 ms and under ε = 1e-3 and 1e-5 alike — the curves coincide; 0.87 at 10⁸ (the float32 floor of the Newton solve). The stiffness is a property of the model, not of the step.",
              "MuJoCo fixed 10 ms: 1.00 at 10³, then 5.7, 57, 572, 5719, 57,188 — ∝ k past the clamp: it realizes at most ~10³ N/m. Fixed 1 ms: 1.00, 1.29, 0.98, 6.8, 68, 681 — realizes up to 10⁵.",
              "MuJoCo error control ε = 1e-3: 1.00, 1.68, 16.8, 168, 1678, 16,782 — the controller's steps (~5 ms) sit above the clamp for k ≥ 10⁴ and the softening is invisible to a position norm (the clamped contact is self-consistent). ε = 1e-5 recovers k = 10⁵ (1.25) and then floors where fixed 1 ms does.",
              "The k = 10³ anchor now reads 1.00 on every MuJoCo line: τ(k) is calibrated through two measured anchors (2.4 ms at 10⁵, 31.8 ms at 10³, τ ∝ k^−0.56) instead of the 1/√k scaling that had left a 1.37× offset."],
        ...
        gaps=["One sentence in the paper on effective-mass scaling (the box in B12 sits 20× stiffer than the sphere with the same τ)."])
    ...
    nb.experiment("B6. Contact artifacts versus cost (figures/artifacts.pdf; penetration_*.pdf; tables/results_tables.md)",
        ...
        says=["Hard clutter, mean penetration (µm) at 10/5/2/1 ms: ICF fixed 18.4 / 17.0 / 9.3 / 9.0; MuJoCo fixed 385 / 116 / 21.6 / 7.7 — both converge toward the model's 6.4 µm. Error control at ε = 1e-1 … 1e-4: ICF 14.2 / 7.4 / 7.5 / 6.3; MuJoCo 504 / 131 / 19.3 / 7.5.",
              "Max penetration (mm): ICF fixed 4.1 / 5.7 / 2.1 / 1.6; MuJoCo fixed 13.4 / 2.8 / 1.5 / 0.7; ICF EC 3.1 / 1.1 / 0.8 / 0.6; MuJoCo EC 10.1 / 2.1 / 0.9 / 0.7. Impact-depth criterion 2.27 mm.",
              "Ejections: none, at any setting of any arm (the wall and dissipation faults of tables/hard_clutter_forensics.md are fixed; the earlier 1.6 % pass-through is gone).",
              "Cheapest artifact-free, hard clutter (wall per simulated second, 64 scenes): MuJoCo fixed 2 ms 0.57 s; MuJoCo EC ε = 1e-2 0.67 s; ICF EC ε = 1e-2 1.97 s; ICF fixed 2 ms 2.48 s — on ICF, error control is the cheaper artifact-free mode. Soft clutter: ICF mean at the model's 0.64 mm (0.65–0.68) everywhere, max ≤ 0.4× the bound; MuJoCo mean 1.5–1.7× the model and max riding the bound itself (no star drawn: the flag straddles).",
              "Both models converge to the same resting compliance; coarse steps are soft in both for different reasons: MuJoCo's clamp (60× the resting depth at 10 ms), ICF's under-resolved impact (2.9×)."],
        not_shown=["On these pure clutter scenes MuJoCo fixed at 2 ms is the cheapest artifact-free arm overall — the figure argues for error control within ICF (ε = 1e-2 beats fixed 2 ms), not across solvers.", "'Penetration' is ground penetration only (body–body overlaps are not read)."],
    ...
    nb.experiment("B9. Measured error versus cost — self-consistency (figures/consistency.pdf)",
        ...
        says=["Floor (reference vs itself, mean dev): soft 4.7 µm ICF / 0.4 µm MuJoCo; hard 1.8 mm ICF / 0.4 mm MuJoCo. A point within a few × of it is noise.",
    ...
    nb.h("B29. 2026-08-30 audit — corrections and supersessions (overrides any earlier B/D number that disagrees)", 1)
    nb.p("Three verification passes ran against the standing chain-v3 CSVs, the instantiated task configs and the public bibliographic record. The generated tables (results_tables, part1_table1, key numbers) regenerate byte-identical from the CSVs; every error found was in hand-written prose. D1 above is refreshed; the corrections that touch dated entries:", italic=True)
    nb.bl(["EC-vs-fixed on the consistency bench was attributed to the WRONG ARM in B9/D1/PART1_LITERATURE: measured, ICF error control sits AT its fixed ladder (1.01-1.14x at matched cost) and MuJoCo error control undercuts its own fixed line 2-3x at tight tolerance.",
           "Ball rebounds (B4): 11 at dt <= 50 us, 12 at 0.1 ms, 13/21 at 0.2/0.5 ms (chatter miscounts at barely-resolved impacts).",
           "Newton tolerance (B14): wall moves <= 5 % only at 1024-4096 worlds; the 64-world 1 ms cells rise ~30 %.",
           "Actuated (B12): ICF tip penetration reaches 0.41 mm (six cells above 0.25); ICF chatter at K_p=1e6 tops at 0.30 m/s (0.36 is a MuJoCo cell); MuJoCo stable-cell lift at 1e5 spans 3.8-21 mm.",
           "Work-precision march budget (B5/A7): the committed wp CSVs ran at max_substeps 65536 (4096 is the ball/actuated budget); exhausted_frac 0 everywhere either way.",
           "Soft-clutter solref conversion (B13/F3): (0.1 s, zeta 0.32) - (1 ms, 3.16) is hard clutter only; the standing soft calibration is tau = 31.8 ms at its own anchor, realized ratio 1.00.",
           "B8's first-bullet scaling numbers and 'spread < 1 % at every N' were stale: standing values 90.8/2290/3960/11,100 ms per boundary at 2^13 (hard), spread up to ~12 % at small N.",
           "B21's 'adaptive ladder run stopped at 600/1000' matches no run on disk - treat as misremembered until a run name is produced.",
           "Campaign as launched 2026-08-30 14:36 (supersedes B17's design): 35 runs = five tasks (slide, lift, plate, flip, tree) x (K1-K5 + adaptive + K5wall), W&B project icra2027, tags exp/<task> + exp/<rung>_dt<ms>; flip/tree adaptive pinned to 1/90x3; K5wall runs at K5's step for the adaptive run's wall budget (step-override bug found and fixed the same day, no wrong run executed).",
           "Flip recipe in the campaign (supersedes B23): 7-stage FSM economy, divergence fine -8 inside it, per-dim init_std scalar 0.05 / (0.05, 2.5), entropy 0, gamma 0.99, obs-norm on, zero-init head, 8 s episodes. Tree: hang FSM (STAGE_C 4.0, paid success bonus 100 - the FSM's 200 constant lives only in the anti-farming assert), init_std 0.5 / (0.05, 3.0), arm scale {0.5 x3, 1.0 x3}, gripper 0.05; divergence exits on tree are UNPRICED (fine covers robot_abnormal only) - open question for Marco.",
           "Representation, measured in the shipped cfgs: the mug is raw per-piece TRIANGLE MESHES in all four mug tasks (fixed by decision in the scene pass); plate = per-piece hulls; dishrack = raw mesh; tree = capsule branches + hull trunk. The 08-25 'hulls everywhere' ruling text is superseded.",
           "Paths: the bench suite and scenes moved to icra2027/part1/{bench,scenes} (every 'newton-adaptive/scripts/bench' reference in B is the pre-08-30 address); cenic-paper is now a symlink to icra2027/paper; the history reports live in cenic-paper-archive/notebook_sources/.",
           "References: all 36 bib entries verified against the public record - none hallucinated; drake14694's real title is 'Implicit PD control in MultibodyPlant' (a stiff-PD issue, consistent with B12's use; refile out of Error-controlled integration or swap for #11132)."])
